...
function CardModal({ team, cards, picksLeft, onClose }: {
  ...
  // All 6 cards belonging to this team
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex flex-col"
        ...
        <div className="flex items-start justify-between px-5 pt-6 pb-4">
          <div>
            <h2 className="display text-5xl text-[var(--gold)]">🎴 MYSTERY CARDS</h2>
            <p className="text-sm text-[var(--muted)] mt-1">
              {opened}/{MAX_CARD_OPENS} opened · {picksLeft} pick{picksLeft !== 1 ? "s" : ""} remaining
            </p>
          ...
          {myCards.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-4 w-full max-w-sm" role="list">
              {myCards.map((c) => {
                ...
                const isPickedRevealing = picked?.id === c.id && phase === "reveal";
                const flipped = !isHidden || isPickedRevealing;
                return (
                  <button key={c.id} role="listitem"
                    disabled={!isHidden || !canPick}
                    onClick={() => choose(c)}
                    aria-label={isHidden ? "Mystery card — tap to pick" : `${c.type} card`}
                    className={`focusable aspect-[3/4] [perspective:800px] ${isHidden && canPick ? "float" : ""} disabled:cursor-default`}
                    ...
                    <div className="relative h-full w-full transition-transform duration-700 [transform-style:preserve-3d]"
                      ...
                      <div className="absolute inset-0 grid place-items-center rounded-2xl border-2 border-[var(--gold)]/70 bg-[linear-gradient(135deg,#3b2a7a,#1c1140)] [backface-visibility:hidden]"
                        style={{ boxShadow: isHidden && canPick ? `0 0 24px -4px ${team.color_hex}` : "none" }}>
                        <span className="text-5xl">🂠</span>
                      </div>

                      {/* Back — revealed */}
                      <div className={`absolute inset-0 grid place-items-center rounded-2xl border-2 p-2 text-center [backface-visibility:hidden] [transform:rotateY(180deg)] ${c.type === "power" ? "border-yellow-300 bg-yellow-400/20" : c.type === "consequence" ? "border-purple-300 bg-purple-500/25" : "border-white/20 bg-white/10"}`}>
                        <span>
        ...
        <div className="px-5 pb-8 text-center">
          {myCards.length > 0 && (
            <p className="text-sm text-[var(--muted)]">
              {canPick
                ? "✨ Tap a glowing card to reveal it"
                : picksLeft === 0
                ? "All picks used for now."
                : `${opened} of ${MAX_CARD_OPENS} cards opened.`}
            </p>
          )}
        </div>
      </div>
      ...
      {phase === "reveal" && result && (
        <div className="fixed inset-0 z-[60] grid place-items-center bg-black/90 px-6 text-center"
          ...
          <div className="pop grid gap-3">
            ...
            <h2 className="display text-5xl sm:text-6xl">{RESULT[result][1]}</h2>
            {picked && result !== "blank" && (
...
export default function Games({ games, team_games, team, cards }: {
  ...
  // Auto-show cards popup when picks become available
  ...
  return (
    <>
      {/* Card modal — full screen when open */}
      {showCards && (
        <CardModal
          team={team}
          cards={cards}
      ...
      <div className="grid gap-3">
        ...
        {totalPicks > 0 && hasEligibleDone && (
          <button onClick={() => setShowCards(true)}
            ...
            <p className="display text-2xl text-[var(--gold)]">
              🎴 {totalPicks} MYSTERY CARD PICK{totalPicks !== 1 ? "S" : ""} WAITING!
            </p>
            <p className="text-xs text-[var(--muted)] mt-1">
              Tap here to open your Mystery Cards now →
            </p>
          </button>
        )}
        ...
        {list.map((g, i) => {
          ...
          const gameHasPicks = s === "completed" && g.power_eligible && totalPicks > 0;
                ...
                {gameHasPicks && (
                  <div className="mt-2 inline-flex items-center gap-1 rounded-full bg-[var(--gold)]/20 border border-[var(--gold)]/50 px-2 py-0.5">
                    ...
                    <span className="text-xs font-bold text-[var(--gold)]">
                      {totalPicks} pick{totalPicks !== 1 ? "s" : ""} waiting — tap above!
                    </span>
                  </div>
                )}
              </button>
                ...
                <div className="border-t border-white/10 px-3 pb-3 pt-2">
                  ...
                  {g.power_eligible && s !== "completed" && (
                    <p className="mt-2 text-xs text-[var(--gold)]">
                      ⚡ Beat the clock to earn Mystery Card picks.
                    </p>
                  )}
