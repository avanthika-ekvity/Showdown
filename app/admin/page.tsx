...
const TEAM_TABS = ["Games", "Cards", "Scores"] as const;
...
function Admin({ staff }: { staff: Staff }) {
  ...
  // Team admins only see their team
  ...
  return (
    <main className="mx-auto max-w-2xl px-3 pb-24 pt-4">
                    ...
                    <li key={g.id} className="flex items-center gap-2 rounded-xl bg-white/5 p-2 text-xs">
                      ...
                      {g.tg?.status === "completed" ? <span>✅</span> : (
                        <div className="flex gap-1 shrink-0 flex-wrap justify-end">
                          {g.time_limit_seconds && <button onClick={() => act("timer_start", { p_team_id: t.id, p_game_id: g.id, p_type: "game", p_seconds: g.time_limit_seconds }, `⏱ Timer started`)} className={`${btn} bg-white/10 text-xs px-2 py-1`}>⏱</button>}
                          {g.power_eligible && <button onClick={() => act("timer_start", { p_team_id: t.id, p_game_id: g.id, p_type: "power_eligibility", p_seconds: elig }, `⚡ Power timer started`)} className={`${btn} bg-yellow-400/80 text-black text-xs px-2 py-1`}>⚡</button>}
                          <button disabled={g.tg?.status === "locked"}
                        ...
                        <div key={s.id} className="mt-1">
                          ...
                          {s.status === "pending" && (
                            <div className="mt-1 flex gap-2">
                              <button onClick={async () => { const ok = await rpc("check_final_guess", { p_key: staff.key, p_submission_id: s.id }); alert(ok ? "✅ Correct word!" : "❌ Wrong word"); }} className={`${btn} bg-white/10 text-xs`}>🔍 Check</button>
          ...
          {visibleTeams.map((t) => {
            ...
            return (
              <div key={t.id} className="arena team-glow p-3" style={{ ["--c" as string]: t.color_hex }}>
                <p className="display text-xl">{t.emoji} {t.name.replace("TEAM ", "")} · {myCards.filter(c => c.status !== "hidden").length}/4 opened · {t.card_chances} picks left</p>
      ...
      {/* TEAMS — master only, with role assignment */}
      {tab === "Teams" && isMaster && (
          ...
          {d.teams.map((t) => {
            ...
            return (
              <div key={t.id} className="arena team-glow p-3" style={{ ["--c" as string]: t.color_hex }}>
                ...
                <form className="mb-3 flex gap-2" onSubmit={(e) => { e.preventDefault(); const f = e.currentTarget.elements.namedItem("code") as HTMLInputElement; act("set_team_code", { p_team_id: t.id, p_code: f.value }, `🔑 Code updated`); f.value = ""; }}>
      ...
      {tab === "Event" && isMaster && d.settings && (
        <section className="mt-4 grid gap-4">
          <div className="arena p-3">
            ...
            <button onClick={() => confirm("Declare winner and end event?") && act("finish_event", {}, "🏆 EVENT COMPLETE!", () => { burst(undefined, true); play("win"); })} className={`${btn} mt-4 w-full bg-[var(--gold)] py-3 text-base text-[#1b1200]`}>🏆 MARK EVENT COMPLETE</button>
          </div>
...
function ScoreControl({ team, act }: { team: Showdown["teams"][number]; act: (fn: string, a: Record<string, unknown>, ok?: string) => Promise<void> }) {
  ...
  const go = (n: number) => { if (!n) return; act("adjust_score", { p_team_id: team.id, p_amount: n, p_reason: reason, p_source: "manual", p_by: "admin" }, `${team.emoji} ${n > 0 ? "+" : ""}${n}`); setAmt(""); };
  return (
...
function ResetAll({ onConfirm }: { onConfirm: () => void }) {
  ...
  return (
    <div className="mt-3 rounded-2xl border border-red-400/60 bg-red-500/10 p-3">
      ...
      <div className="mt-2 flex gap-2">
        <input value={v} onChange={(e) => setV(e.target.value)} className="focusable flex-1 rounded-lg bg-black/30 px-2 py-1 text-sm" />
