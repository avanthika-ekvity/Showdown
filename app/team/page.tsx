...
export default function TeamPage() {
  ...
  const finalTg = d.team_games.find(
    (t) => t.team_id === team.id && t.game_id === "final",
  );
  const cheer =
    rank === 0
      ? "Top spot tumhara hai. 👀"
      : rank === 1
        ? "Ek push aur. Aage badho!"
        : "Score check karo. Comeback time!";

  return (
    <main
      className="mx-auto max-w-2xl px-3 pb-24 pt-4"
      style={{ ["--c" as string]: team.color_hex }}
    >
      {toast && (
        <div
          role="status"
          className="slam fixed inset-x-3 top-3 z-40 rounded-2xl bg-[var(--gold)] px-4 py-2 text-center text-[#1b1200] shadow-2xl"
        >
      ...
      {tab === "home" && (
        <div className="mt-4 grid gap-3">
            ...
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="display text-5xl">
                  <span key={team.score} className="inline-block bump">
                    {team.score}
                  </span>{" "}
          ...
          {/* Live game */}
          <section
            className={`arena flex items-center gap-3 p-3 ${live ? "border-red-400/60" : ""}`}
          >
            <span
              className={`grid h-11 w-11 shrink-0 place-items-center rounded-full text-2xl ${live ? "live-dot bg-[var(--red)]" : "bg-white/10"}`}
              style={{ ["--c" as string]: "var(--red)" }}
            >
              {live?.emoji ?? current?.emoji ?? "🏁"}
            </span>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold tracking-widest text-[#ff8a80]">
                {live ? "🔴 LIVE NOW" : current ? "UP NEXT" : "ALL DONE"}
              </p>
              <p className="display truncate text-2xl">
                {live?.name ?? current?.name ?? "Final Showdown awaits"}
              </p>
              <p className="text-xs text-[var(--muted)]">
                {done}/{list.length} games done
              </p>
            </div>
            <button
              onClick={() => setTab("games")}
              className="focusable shrink-0 rounded-full bg-white/10 px-3 py-1.5 text-xs"
            >
              Rules →
            </button>
          </section>
          ...
          {finalTg && finalTg.status !== "locked" && (
            <Final
              team={team}
              teams={d.teams}
              submissions={d.final_submissions.filter(
                (s) => s.team_id === team.id,
              )}
          ...
          {team.card_chances > 0 && (
            <button
              ...
              className="focusable arena border-[var(--gold)] p-3 text-left live-dot"
              style={{ ["--c" as string]: "var(--gold)" }}
            >
              <p className="display text-2xl text-[var(--gold)]">
                ⚡ {team.card_chances} CARD PICK
                {team.card_chances > 1 ? "S" : ""} WAITING!
              </p>
              <p className="text-xs mt-1">
                Fate is in your hands. Pick a Mystery Card →
              </p>
            </button>
          )}
        </div>
      ...
      {tab === "games" && (
        <div className="mt-4">
          ...
          <p className="mb-3 text-xs text-[var(--muted)]">
            Play in order. Admin marks each one done and the next unlocks.
          </p>
      ...
      {tab === "team" && (
        <div className="mt-4">
          ...
          <ul className="grid grid-cols-2 gap-2">
            {d.members
              ...
              .map((m) => (
                <li
                  key={m.id}
                  className={`arena rounded-xl p-3 text-sm ${m.role === "captain" ? "border-[var(--gold)]/60 bg-[var(--gold)]/10" : m.role === "power_holder" ? "border-yellow-300/40 bg-yellow-300/10" : m.role === "consequence_holder" ? "border-purple-300/40 bg-purple-400/10" : ""}`}
                >
          ...
          <button
            ...
            className="focusable mt-5 text-xs text-[var(--muted)] underline-offset-4 hover:underline"
          >
            Switch team
          </button>
        </div>
