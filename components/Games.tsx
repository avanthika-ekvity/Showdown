...
function CardModal({ team, cards, picksLeft, onClose }: {
  ...
  const canPick = picksLeft > 0
    && sessionOpened < SESSION_LIMIT
    && totalOpened < MAX_CARD_OPENS
    && phase === "idle";
  ...
  async function choose(c: Card) {
    ...
    try {
      ...
      setPhase("reveal");
      if (type === "power") {
        burst(team.color_hex, true); play("power"); buzz([80, 40, 80]);
      } else {
        play("oops"); buzz(type === "consequence" ? 300 : 50);
      }
    } catch (e) {
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: `radial-gradient(100vw 80vh at 50% 30%, ${team.color_hex}55, var(--bg) 70%)`,
        }}
      >
        {/* Header */}
        <div className="flex items-start justify-between px-5 pt-6 pb-4">
          <div>
            ...
            {err && (
              <p role="alert" className="text-sm text-[#ffb3ad] mt-1">{err}</p>
            )}
          </div>
          <button
            onClick={onClose}
          ...
          {myCards.length === 0 ? (
            <div className="text-center">
              ...
              <p className="text-sm text-[var(--muted)] mt-2">
                Ask the admin to check the cards setup.
              </p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {/* Front — face down */}
                      <div
                        className="absolute inset-0 grid place-items-center rounded-2xl border-2 border-[var(--gold)]/70 bg-[linear-gradient(135deg,#3b2a7a,#1c1140)] [backface-visibility:hidden]"
                        style={{
                          boxShadow: isPickable ? `0 0 28px -4px ${team.color_hex}` : "none",
                        }}
                      >
                      ...
                      {/* Back — revealed */}
                      <div
                        className={`absolute inset-0 grid place-items-center rounded-2xl border-2 p-2 text-center [backface-visibility:hidden] [transform:rotateY(180deg)] ${
                          c.type === "power"
        ...
        <div className="px-5 pb-8 text-center grid gap-3">
          <p className="text-sm text-[var(--muted)]">
            {canPick
              ? `✨ Tap a glowing card · ${SESSION_LIMIT - sessionOpened} pick${SESSION_LIMIT - sessionOpened !== 1 ? "s" : ""} left this round`
              : sessionOpened >= SESSION_LIMIT
      ...
      {phase === "count" && (
        <div className="fixed inset-0 z-[60] grid place-items-center bg-black/90 text-center">
          <div>
            ...
            <p
              key={count}
              className="display slam"
              style={{ fontSize: "clamp(8rem, 40vw, 18rem)" }}
            >
              {count}
            </p>
          </div>
      ...
      {phase === "reveal" && result && (
        <div
          className="fixed inset-0 z-[60] grid place-items-center bg-black/90 px-6 text-center"
  ...
  return (
    <>
      ...
      <div className="grid gap-3">
        {/* Card picks banner — shows when picks are available */}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {isLocked ? (
                      <span className="display block text-2xl blur-sm select-none text-[var(--muted)]">
                        ???? ??????
                      </span>
                    ) : (
                  ...
                  <span className="display shrink-0 text-right text-lg leading-tight">
                    ...
                    <span className="text-xs">
                      {s === "completed"
                        ? `+${g.tg?.points_awarded ?? g.points_awarded}`
                        : label}
                    </span>
                  </span>
                </div>
                ...
                <div className="border-t border-white/10 px-3 pb-3 pt-2">
                  ...
                  {g.power_eligible && s !== "completed" && (
                    <p className="mt-2 text-xs text-[var(--gold)]">
                      ⚡ Beat the clock to earn 2 Mystery Card picks.
                    </p>
                  )}
        ...
        {/* Final showdown row */}
        <div
          className={`arena flex items-center gap-3 border p-3 ${
            final?.status === "locked"
              ? "border-[var(--gold)]/30 opacity-60"
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="display block text-2xl">
              {finalGame?.name ?? "THE FINAL SHOWDOWN"}
            </span>
            <span className="block text-xs text-[var(--muted)]">
              {final?.status === "locked"
                ? "Clear all games to unlock"
                : "Unlocked. Find the admin."}
            </span>
          </span>
        </div>
      </div>
