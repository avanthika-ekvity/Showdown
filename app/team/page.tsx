        ...
        <div className="mt-4 grid gap-3">
          ...
          <section className="arena team-glow p-4">
            <div className="flex items-end justify-between">
              <div><p className="text-xs text-[var(--muted)]">SCORE</p><p className="display text-5xl"><span key={team.score} className="inline-block bump">{team.score}</span> <span className="text-2xl text-[var(--muted)]">STEPS</span></p></div>
              <div className="text-right"><p className="text-xs text-[var(--muted)]">RANK</p><p className="display text-5xl">{RANK_LABEL[rank]}</p></div>
            </div>
            <p className="mt-1 text-xs">Checkpoint {team.board_position}/20 · {team.board_position * 5}%</p>
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full transition-all duration-700" style={{ width: `${team.board_position * 5}%`, background: team.color_hex }} /></div>
          ...
          <section>
            ...
            <p className="text-xs text-[var(--muted)] mb-3">Play in order. Admin marks each one done.</p>
            <Games games={d.games} team_games={d.team_games} team={team} cards={d.cards} />
          </section>
        </div>
      )}
      ...
      {tab === "team" && (
        <div className="mt-4">
          ...
          <p className="mb-3 text-xs text-[var(--muted)]">
            {d.members.some(m => m.team_id === team.id && m.role !== "player") ? "Roles assigned by admin." : "Roles will be decided on the day."}
          </p>
          <ul className="grid grid-cols-2 gap-2">
            {d.members
              ...
              .map((m) => (
                <li key={m.id} className={`arena rounded-xl p-3 text-sm ${m.role === "captain" ? "border-[var(--gold)]/60 bg-[var(--gold)]/10" : m.role === "power_holder" ? "border-yellow-300/40 bg-yellow-300/10" : m.role === "consequence_holder" ? "border-purple-300/40 bg-purple-400/10" : ""}`}>
                  <span className="block text-xs text-[var(--muted)]">
                    {m.role === "captain" ? "👑 Captain" : m.role === "power_holder" ? "⚡ Power Holder" : m.role === "consequence_holder" ? "😈 Consequence Holder" : "🎮 Player"}
                  </span>
                  <span className="font-semibold">{m.name.split(" ")[0]}</span>
                  <span className="block text-xs text-[var(--muted)]">{m.name.split(" ").slice(1).join(" ")}</span>
                </li>
              ))}
          </ul>
          <p className="mt-3 text-xs text-[var(--muted)]">{members.length} members total</p>
          <button onClick={() => { if (confirm("Log out of this team?")) { localStorage.removeItem(TEAM_KEY); router.replace("/enter"); } }} className="focusable mt-3 text-xs text-[var(--muted)] underline-offset-4 hover:underline">Switch team</button>
        </div>
      )}
