...
function InlineCards({ team, cards, picksLeft }: { team: Team; cards: Card[]; picksLeft: number }) {
  ...
  return (
    <div className="mt-3 rounded-2xl border border-[var(--gold)]/60 bg-[var(--gold)]/10 p-3">
      ...
      <p className="text-xs text-[var(--muted)] mt-0.5">{opened}/{MAX_CARD_OPENS} opened · {picksLeft} pick{picksLeft !== 1 ? "s" : ""} left</p>
          ...
          return (
            <button key={c.id} role="listitem"
              ...
              <div className="relative h-full w-full transition-transform duration-700 [transform-style:preserve-3d]"
                ...
                <div className="absolute inset-0 grid place-items-center rounded-xl border-2 border-[var(--gold)]/60 bg-[linear-gradient(135deg,#3b2a7a,#1c1140)] text-xl shadow-[0_0_16px_-6px_var(--gold)] [backface-visibility:hidden]">🂠</div>
                <div className={`absolute inset-0 grid place-items-center rounded-xl border-2 p-1 text-center text-[10px] [backface-visibility:hidden] [transform:rotateY(180deg)] ${c.type === "power" ? "border-yellow-300 bg-yellow-400/20" : c.type === "consequence" ? "border-purple-300 bg-purple-500/25" : "border-white/30 bg-white/10"}`}>
                  <span><span className="block text-lg">{c.type === "power" ? "⚡" : c.type === "consequence" ? "😈" : "😶"}</span><span className="block leading-tight mt-0.5">{c.name.replace(/[🥷❄️🤫😳😶]\s?/g, "")}</span></span>
                </div>
              </div>
            </button>
          );
      ...
      {phase === "count" && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/85 text-center">
          <div><p className="display slam text-2xl text-[var(--gold)]">THE CARD HAS SPOKEN...</p><p key={count} className="display slam text-[35vw] sm:text-[10rem]">{count}</p></div>
        </div>
      )}
...
export default function Games({ games, team_games, team, cards }: { games: Game[]; team_games: TeamGame[]; team: Team; cards: Card[] }) {
  ...
  return (
    <div className="grid gap-3">
      {list.map((g, i) => {
        ...
        const picksGranted = g.tg?.card_picks_granted ?? 0;
        const showCards = s === "completed" && picksGranted > 0 && totalPicks > 0;
        ...
        return (
          <div key={g.id} className={`arena border overflow-hidden transition-all ${cls || "border-white/10"} ${s === "live" ? "live-dot" : ""}`} style={{ ["--c" as string]: "var(--red)" }}>
            <button onClick={() => !isLocked && setExpanded(isOpen ? null : g.id)} disabled={isLocked}
              className={`focusable w-full p-3 text-left ${isLocked ? "cursor-default" : ""}`}>
              <div className="flex items-center gap-3">
                <span className={`text-3xl ${isLocked ? "grayscale opacity-40" : ""}`}>{isLocked ? "🔒" : g.emoji}</span>
                <span className="min-w-0 flex-1">
                  <span className="block text-xs text-[var(--muted)]">GAME {i + 1}{g.assigned_team_id ? " · YOUR EXCLUSIVE" : ""}</span>
                  {isLocked
                  ...
                  <span className="block text-xs text-[var(--muted)]">{isLocked ? `Complete Game ${i} to unlock` : g.tagline}</span>
                </span>
                <span className="display shrink-0 text-right text-lg leading-tight">
                  {icon}<br /><span className="text-xs">{s === "completed" ? `+${g.tg?.points_awarded ?? g.points_awarded}` : label}</span>
                </span>
              </div>
              {/* Card picks badge */}
              {showCards && <div className="mt-2 inline-flex items-center gap-1 rounded-full bg-[var(--gold)]/20 border border-[var(--gold)]/50 px-2 py-0.5"><span className="text-xs">🎴</span><span className="text-xs font-bold text-[var(--gold)]">{totalPicks} card pick{totalPicks !== 1 ? "s" : ""} available!</span></div>}
            </button>
            ...
            {isOpen && !isLocked && (
              <div className="border-t border-white/10 px-3 pb-3 pt-2">
                ...
                {g.rules && (<><p className="display mt-3 text-lg">RULES</p><ol className="mt-1 list-decimal space-y-1 pl-5 text-xs">{g.rules.split("\n").filter(Boolean).map((r, i) => <li key={i}>{r}</li>)}</ol></>)}
                {g.twist && (<div className="mt-3 rounded-xl border border-orange-400/50 bg-orange-500/10 p-2"><p className="text-xs text-orange-200 font-bold">SPECIAL TWIST</p><p className="text-sm">{g.twist}</p></div>)}
                {g.team_config?.[team.id] && (<div className="mt-2 rounded-xl bg-white/10 p-2"><p className="text-xs text-[var(--muted)]">{team.emoji} YOUR CHALLENGE</p><p className="text-sm font-semibold">{g.team_config[team.id]}</p></div>)}
                {g.power_eligible && <p className="mt-2 text-xs text-[var(--gold)]">⚡ Beat the clock to earn Mystery Card picks.</p>}
                <p className="mt-3 rounded-xl bg-black/30 p-2 text-center text-xs text-[var(--muted)]">Complete the challenge. Admin marks it done.</p>
                {/* Inline card picker */}
                {showCards && <InlineCards team={team} cards={cards} picksLeft={totalPicks} />}
              </div>
            )}

            {/* Card picker also visible when collapsed but picks available */}
            {!isOpen && showCards && (
