...
const RESULT = {
  power: [
    "⚡",
    "POWER ACQUIRED!",
    "Your Power is ready. Find the admin to activate it.",
  ],
  consequence: [
    "😈",
    "FATE HAS SPOKEN.",
    "You wanted The Showdown. The Showdown wanted YOU.",
  ],
  blank: [
    "😶",
    "BETTER LUCK NEXT TIME",
    "No power. No consequence. Just vibes.",
  ],
} as const;
...
export default function Cards({
  ...
  const canOpen =
    team.card_chances > 0 && opened < MAX_CARD_OPENS && phase === "idle";
  ...
  async function choose(c: Card) {
    ...
    try {
      const type = (await rpc("select_card", {
        p_team_id: team.id,
        p_card_id: c.id,
      })) as Card["type"];
      setResult(type);
      setPhase("reveal");
      if (type === "power") {
        burst(team.color_hex, true);
        play("power");
        buzz([80, 40, 80]);
      } else {
        play("oops");
        buzz(type === "consequence" ? 300 : 50);
      }
    } catch (e) {
    ...
    <div className="grid gap-4">
      <section className="arena p-4">
        ...
        <p className="text-xs text-[var(--muted)] mt-1">
          {opened}/{MAX_CARD_OPENS} opened · {team.card_chances} pick
          {team.card_chances !== 1 ? "s" : ""} remaining
        </p>

        {team.card_chances > 0 && opened < MAX_CARD_OPENS ? (
          <p className="display mt-2 text-xl text-[var(--gold)]">
            🎴 YOU'VE EARNED A PICK! Choose wisely...
          </p>
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">
            Beat the clock in 🌶️ Mirchi Mayhem or 💧 Guess The Jal to earn
            picks.
          </p>
        )}

        {err && (
          <p role="alert" className="mt-2 text-sm text-[#ffb3ad]">
            {err}
          </p>
        )}
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Back: revealed */}
                  <div
                    className={`absolute inset-0 grid place-items-center rounded-xl border-2 p-1 text-center text-xs [backface-visibility:hidden] [transform:rotateY(180deg)] ${c.type === "power" ? "border-yellow-300 bg-yellow-400/20" : c.type === "consequence" ? "border-purple-300 bg-purple-500/25" : "border-white/30 bg-white/10"}`}
                  >
                    <span>
      ...
      {phase === "count" && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/85 text-center">
          <div>
            <p className="display slam text-2xl text-[var(--gold)]">
              THE CARD HAS SPOKEN...
            </p>
            <p key={count} className="display slam text-[35vw] sm:text-[10rem]">
              {count}
            </p>
          </div>
        </div>
      ...
      {phase === "reveal" && result && (
        <div
          className="fixed inset-0 z-50 grid place-items-center bg-black/85 px-6 text-center"
          onClick={() => {
            setPhase("idle");
            setPicked(null);
            setResult(null);
      ...
      <section className="arena border-yellow-300/40 p-4">
        ...
        {myPowers.map((p) => (
          <div
            key={p.id}
            className={`mt-2 rounded-xl bg-white/5 p-3 relative overflow-hidden ${p.status === "locked" ? "select-none" : ""}`}
          >
            {p.status === "locked" && (
              <div className="absolute inset-0 backdrop-blur-md bg-black/40 z-10 flex items-center justify-center rounded-xl">
            ...
            {p.status === "available" && (
              <p className="mt-1 text-xs text-[var(--gold)]">
                Ready. Find the admin to activate.
              </p>
            )}
            {p.status === "used" && (
              <p className="mt-1 text-xs text-[var(--muted)]">Already fired.</p>
            )}
          </div>
        ))}
      </section>
      ...
      <section className="arena border-purple-300/40 p-4">
        ...
        {myCons.map((c) => (
          <div
            key={c.id}
            className={`mt-2 rounded-xl bg-white/5 p-3 relative overflow-hidden ${c.status === "locked" ? "select-none" : ""}`}
          >
            {c.status === "locked" && (
              <div className="absolute inset-0 backdrop-blur-md bg-black/40 z-10 flex items-center justify-center rounded-xl">
                <span className="text-2xl">🔒</span>
            ...
            {c.status === "active" && (
              <p className="mt-1 text-xs text-[#ff8a80] font-bold">
                😈 Serve it now.
              </p>
            )}
          </div>
