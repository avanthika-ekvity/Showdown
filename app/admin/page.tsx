...
const TABS = ["Live", "Games", "Scores", "Cards", "Teams", "Event", "History"] as const;
...
function Admin({ staffKey }: { staffKey: string }) {
  ...
  async function act(fn: string, args: Record<string, unknown>, ok = "Saved") {
    try { await rpc(fn, { p_key: staffKey, ...args }); setMsg(ok); }
    ...
    <main className="mx-auto max-w-2xl px-3 pb-24 pt-4">
      ...
      <header className="flex items-center justify-between">
        ...
        <div className="flex gap-2 text-xs">
          <Link href="/tv" className={`${btn} border border-white/20`}>📺 TV</Link>
          <Link href="/qr" className={`${btn} border border-white/20`}>📱 QR</Link>
          <button onClick={logoutStaff} className={`${btn} border border-white/20`}>Lock</button>
        </div>
      </header>

      {/* Tab bar */}
      <nav className="mt-3 flex gap-1 overflow-x-auto pb-1" aria-label="Admin sections">
        {TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)} aria-current={tab === t}
            className={`${btn} shrink-0 text-xs ${tab === t ? "bg-[var(--gold)] text-[#1b1200]" : "bg-white/10"}`}>{t}</button>
        ))}
      </nav>
      ...
      {tab === "Games" && (
        <section className="mt-4 grid gap-3">
          ...
          {teams.map((t) => {
            ...
            return (
              <details key={t.id} className="arena team-glow p-3" style={{ ["--c" as string]: t.color_hex }}>
                      ...
                      {g.tg?.status === "completed"
                        ...
                        : <div className="flex gap-1 shrink-0">
                            {g.time_limit_seconds && (
                              <button onClick={() => act("timer_start", { p_team_id: t.id, p_game_id: g.id, p_type: "game", p_seconds: g.time_limit_seconds }, `⏱ ${g.name} timer started`)} className={`${btn} bg-white/10 text-xs px-2 py-1`}>⏱</button>
                            )}
                            {g.power_eligible && (
                              <button onClick={() => act("timer_start", { p_team_id: t.id, p_game_id: g.id, p_type: "power_eligibility", p_seconds: elig }, `⚡ Power timer started`)} className={`${btn} bg-yellow-400/80 text-black text-xs px-2 py-1`}>⚡</button>
                            )}
                            <button
                              disabled={g.tg?.status === "locked"}
                              onClick={() => confirm(`Mark ${g.name} complete for ${t.name}? +${g.points_awarded} steps.`) &&
                                act("complete_game", { p_team_id: t.id, p_game_id: g.id }, `🎉 GAME CRUSHED! +${g.points_awarded}`, () => { burst(t.color_hex, true); play("win"); })}
                              className={`${btn} bg-green-500 text-black text-xs px-2 py-1`}>✅</button>
                          </div>}
                    </li>
                  ))}
                </ul>
                      ...
                      {timer.timer_type === "power_eligibility" && (
                        <button onClick={() => {
                          ...
                          act("timer_action", { p_id: timer.id, p_action: "complete" }, "")
                            .then(() => act("grant_card_chance", { p_team_id: t.id, p_delta: chances }, `⚡ +${chances} card picks unlocked!`, () => { burst("#f5c542"); play("power"); }));
                        }} className={`${btn} bg-yellow-400 text-black text-xs`}>⚡ Beat clock → +{CARD_CHANCES_PER_GAME[timer.game_id ?? ""] ?? 2} picks</button>
                      )}
                    </div>
                  </div>
                ))}
                  ...
                  <div key={p.id} className="mt-2 flex flex-wrap items-center gap-2 rounded-xl bg-yellow-400/10 border border-yellow-300/40 p-2 text-xs">
                    ...
                    <button onClick={() => {
                      ...
                      rpc("request_power", { p_team_id: t.id, p_power_id: p.id, p_target_team_id: tgt }).catch(() => {})
                        .then(() => act("use_power", { p_power_id: p.id }, p.name.includes("STEAL") ? `✂️ ${t.name} stole 5 steps!` : "❄️ TIME FROZEN!", () => { burst(t.color_hex); play("power"); }));
                        ...
                        <div key={s.id} className="mt-1">
                          ...
                          {s.status === "pending" && (
                            <div className="mt-1 flex gap-2">
                              <button onClick={async () => { const ok = await rpc("check_final_guess", { p_key: staffKey, p_submission_id: s.id }); alert(ok ? "✅ Correct word!" : "❌ Wrong word"); }} className={`${btn} bg-white/10 text-xs`}>🔍 Check</button>
                ...
                {/* Card chances nudge */}
                <p className="mt-2 text-xs text-[var(--muted)]">Card picks: <b className="text-[var(--gold)]">{t.card_chances}</b>
          ...
          {d.teams.map((t) => {
            ...
            return (
              <div key={t.id} className="arena team-glow p-3" style={{ ["--c" as string]: t.color_hex }}>
                <p className="display text-xl">{t.emoji} {t.name.replace("TEAM ", "")} · {myCards.filter(c => c.status !== "hidden").length}/4 opened</p>
                <div className="mt-2 grid grid-cols-3 gap-1">
                  {myCards.map(c => (
                    <div key={c.id} className={`rounded-lg p-2 text-xs text-center ${c.type === "power" ? "bg-yellow-400/20 border border-yellow-300/50" : c.type === "consequence" ? "bg-purple-500/20 border border-purple-300/50" : "bg-white/5"}`}>
                      <p className="font-bold">{c.type === "power" ? "⚡" : c.type === "consequence" ? "😈" : "😶"}</p>
                      <p className="text-[10px] text-[var(--muted)]">{c.status}</p>
                      {c.status !== "hidden" && <button onClick={() => update("cards", c.id, { status: "hidden", used: false })} className="mt-1 text-[10px] underline">reset</button>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          {d.teams.map((t) => (
            <div key={t.id} className="arena team-glow p-3" style={{ ["--c" as string]: t.color_hex }}>
              <div className="flex items-center gap-2">
                <Ed table="teams" id={t.id} field="emoji" value={t.emoji} cls="text-2xl w-10" />
                <Ed table="teams" id={t.id} field="name" value={t.name} cls="display text-2xl flex-1" />
                <input type="color" aria-label="Team color" defaultValue={t.color_hex} onBlur={(e) => e.target.value !== t.color_hex && update("teams", t.id, { color_hex: e.target.value })} className="h-8 w-10 rounded" />
              </div>
              <form className="mt-2 flex gap-2" onSubmit={(e) => { e.preventDefault(); const f = e.currentTarget.elements.namedItem("code") as HTMLInputElement; act("set_team_code", { p_team_id: t.id, p_code: f.value }, `🔑 Code updated`); f.value = ""; }}>
        ...
        <section className="mt-4 grid gap-4">
          <div className="arena p-3">
            ...
            {([["event_name", d.settings.event_name], ["tagline", d.settings.tagline], ["power_eligibility_seconds", d.settings.power_eligibility_seconds], ["final_bonus_points", d.settings.final_bonus_points]] as const).map(([f, v]) => (
              <label key={f} className="mt-2 grid gap-1 text-xs"><span className="text-[var(--muted)]">{f}</span><Ed table="event_settings" id={1} field={f} value={v} cls="rounded-lg bg-black/30 px-2 py-1" /></label>
            ))}
            <label className="mt-2 grid gap-1 text-xs"><span className="text-[var(--muted)]">event_status</span>
            ...
            <button onClick={() => confirm("Declare winner and end event?") && act("finish_event", {}, "🏆 EVENT COMPLETE!").then(() => { burst(undefined, true); play("win"); })} className={`${btn} mt-4 w-full bg-[var(--gold)] py-3 text-base text-[#1b1200]`}>🏆 MARK EVENT COMPLETE</button>
          </div>
  ...
  return (
    <div className="arena team-glow p-3" style={{ ["--c" as string]: team.color_hex }}>
      ...
      <div className="mt-2 flex gap-2">
        <input type="number" value={amt} onChange={(e) => setAmt(e.target.value)} placeholder="±custom" aria-label="Custom amount" className="focusable w-20 rounded-lg bg-black/30 px-2 py-1 text-sm" />
        <button onClick={() => go(Number(amt))} className={`${btn} bg-white/10 text-xs`}>Apply</button>
      </div>
      <select value={reason} onChange={(e) => setReason(e.target.value)} aria-label="Reason" className="focusable mt-2 w-full rounded-lg bg-black/30 px-2 py-1 text-xs">
